...
// stable business Code (e.g. "G-1", "G-2", ...). One row per entity type.
...
// ordered by orderBy, so pre-existing rows (from before Code existed) get
// one without disturbing already-coded rows. Idempotent — safe to call on
// every Migrate.
//
// orderBy names a column that yields a stable, deterministic sequence.
// created_at is the natural choice where a row carries one (ActorRow,
// GroupRow), preserving chronological code order; HierarchyRow, LevelRow,
// and RoleKindRow have no created_at column at all, so their callers pass
// "id" instead — an arbitrary but still deterministic and idempotent order,
// fine for a one-time backfill of rows that predate Code.
func BackfillCodes(db *gorm.DB, table, sequenceTable, entityType, prefix, orderBy string) error {
	return db.Transaction(func(tx *gorm.DB) error {
		var ids []string
		if err := tx.Table(table).Where("code = '' OR code IS NULL").Order(orderBy).Pluck("id", &ids).Error; err != nil {
